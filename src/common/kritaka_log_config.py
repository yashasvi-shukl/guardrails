...
from src.common.kritaka_decorators import singleton
...
class LogConfiguration:
    ...
    """
    # ---------------------------------------------------------------------------------------------
    __DEFAULT_APPLICATION_NAME: str = 'kritaka-ragaas'
    ...
    def setup_logging(self, level: int =logging.INFO):
        ...
        if self.with_file_logging:
            ...
            #logging.basicConfig(filename=self.log_file_path,force=True)
...
if __name__ == "__main__":
    ...
    log_config = LogConfiguration(application_name = 'kritaka-ragaas',
                                    with_console_logging=True, 
                                    with_file_logging=True,
                                    log_file_path='./var/log/kritaka/ragaas/ragaas.log')
    log_config.set_level(logging.INFO)

    # Now you can use the logger within your main application
    logging.basicConfig(filename=log_config.log_file_path,level=logging.INFO,format=log_config.log_format,force=True)
    logger = logging.getLogger('kritaka-ragaas')
    logger.error("Application is starting...")
